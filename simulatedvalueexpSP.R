...
Criterion<-function(a) {
  ...
  for (i in 1:((length(a)-1)/3)){
    ...
    dif_dummy = dif_dummy + a[3*i-1]*SoftPlus_dif(a[3*i]*S+a[3*i+1])*a[3*i]
  }
...
Gradient <- function(a){
  ...
  for (j in 1:((length(a)-1)/3)){
    ...
    dif_dummy = dif_dummy+a[3*j-2]*SoftPlus_dif(a[3*j-1]*S + a[3*j])*a[3*j-1]
  ...
  for (j in 1:((length(a)-1)/3)){
    ...
    dif_d1 = SoftPlus_dif(a[3*j-1]*S+a[3*j])*a[3*j-1]
    Grad[3*j-1] = w*2*sum(Dummy*d1)+(1-w)*2*sum(dif_dummy*dif_d1)
    
    d2 = a[3*j-2]*SoftPlus_dif(a[3*j-1]*S + a[3*j])*S
    dif_d2 = a[3*j-2]*SoftPlus_difdif(a[3*j-1]*S+a[3*j])*S*a[3*j-1]+a[3*j-2]*SoftPlus_dif(a[3*j-1]*S+a[3*j])
    Grad[3*j] = w*2*sum(Dummy*d2)+(1-w)*2*sum(dif_dummy*dif_d2)
    
    d3 = a[3*j-2]*SoftPlus_dif(a[3*j-1]*S + a[3*j])
    dif_d3 = a[3*j-2]*SoftPlus_difdif(a[3*j-1]*S+a[3*j])*a[3*j-1]
    Grad[3*j+1] = w*2*sum(Dummy*d3)+(1-w)*2*sum(dif_dummy*dif_d3)
  }
  return(Grad)
}
if (length(x) == 4){
  y = SoftPlus_reg(res$par,S)
  delta = res$par[2]*SoftPlus_dif(res$par[3]*S+res$par[4])*res$par[3]
} else if (length(x)==7){
  y = SoftPlus_2reg(res$par,S)
  delta = res$par[2]*SoftPlus_dif(res$par[3]*S+res$par[4])*res$par[3]+res$par[5]*SoftPlus_dif(res$par[6]*S+res$par[7])*res$par[6]
} else if (length(x)==10){
  y = SoftPlus_3reg(res$par,S)
  delta = res$par[2]*SoftPlus_dif(res$par[3]*S+res$par[4])*res$par[3]+res$par[5]*SoftPlus_dif(res$par[6]*S+res$par[7])*res$par[6]+res$par[8]*SoftPlus_dif(res$par[9]*S+res$par[10])*res$par[9]
} else if (length(x)==13){
  y = SoftPlus_4reg(res$par,S)
  delta = res$par[2]*SoftPlus_dif(res$par[3]*S+res$par[4])*res$par[3]+res$par[5]*SoftPlus_dif(res$par[6]*S+res$par[7])*res$par[6]+res$par[8]*SoftPlus_dif(res$par[9]*S+res$par[10])*res$par[9]+res$par[11]*SoftPlus_dif(res$par[12]*S+res$par[13])*res$par[12]
} else if (length(x)==16){
  y = SoftPlus_5reg(res$par,S)
  delta = res$par[2]*SoftPlus_dif(res$par[3]*S+res$par[4])*res$par[3]+res$par[5]*SoftPlus_dif(res$par[6]*S+res$par[7])*res$par[6]+res$par[8]*SoftPlus_dif(res$par[9]*S+res$par[10])*res$par[9]+res$par[11]*SoftPlus_dif(res$par[12]*S+res$par[13])*res$par[12]+res$par[14]*SoftPlus_dif(res$par[15]*S+res$par[16])*res$par[15]
}
