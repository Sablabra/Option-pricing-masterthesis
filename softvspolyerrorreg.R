...
order = 14
Price_errorsoftnoreg = matrix(0,order,1)
Delta_errorsoftnoreg = matrix(0,order,1)
for (h in 1:order){
  if (h==1){
    x = c(0,0,0,0)
  } else {
  ...
  Price_errorsoftnoreg[h] = round(mean(abs(y-TruePrice)),8)
  Delta_errorsoftnoreg[h] = round(mean(abs(delta-TrueDelta)),8)
}
